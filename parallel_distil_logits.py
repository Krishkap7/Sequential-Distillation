...
def run_parallel_logits(
    ...
    for student in students:
        ...
        output_dir = os.path.join(base_output_dir, f"{student_name}_from_{sanitize_model_id(teacher)}_logits")

        cfg = build_logits_config(
            teacher=teacher,
            student=student,
            output_dir=output_dir,
        )
        run_logit_distillation(cfg)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Logit-based parallel distillation: a single teacher distills into one or "
    ...
    parser.add_argument(
        "--base-output-dir",
        type=str,
        default="./parallel_checkpoints/logits",
